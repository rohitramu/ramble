...
if [ -d /root/.spack ] && [ ! -d "$HOME/.spack" ]; then
  cp -rn /root/.spack "$HOME/.spack" 2>/dev/null || true
fi

# Load py-pip with retries against transient git/network failures
spack_loaded=0
for attempt in 1 2 3; do
  if spack load py-pip ^python; then
    spack_loaded=1
    break
  fi
  echo "WARNING: 'spack load py-pip ^python' failed on attempt $attempt of 3."
